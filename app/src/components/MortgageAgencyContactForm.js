...
export default class MortgageAgencyContactForm extends Component {
    ...
    submitHandler = (e) => {
        e.preventDefault();
        const {name, email, phone, bankId, offer, amount, creditPeriod, age, chooseServiceCharge, chooseInsurance, contributionPercent, cost} = this.state;
        ...
        if (readyName && readyEmail && readyPhone && readyAgree && captchaValue !== '') {
            window.location.href = `/mortgageAgencyContactFormSend/${bankId}/${offer}/${amount}/${creditPeriod}/${age}/${chooseServiceCharge}/${chooseInsurance}/${contributionPercent}/${cost}/${name}/${email}/${phone}`;
        }
    }

    render() {
        const {logo, banks, bankId, offer, amount, creditPeriod, age, chooseServiceCharge, chooseInsurance, contributionPercent, cost} = this.state;
